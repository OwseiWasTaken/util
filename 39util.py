...
def near(
	...
	num: Union[float, int],
	DifUp: Union[float, int],
	DifDown: Union[float, int] = 0,
) -> bool:
	if not DifDown:
		DifDown = DifUp
...
def ORGroups(
	g1: Union[list, tuple, set, frozenset], g2: Union[list, tuple, set, frozenset]
) -> set:
	Gret = set((*g1, *g2))
	return Gret
...
?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_´`abcdefghijklmnopqrstuvwxyz{|}~"
	...
	def loop(this) -> str:
		...
		if this.DrawRect:
			DrawRectangle(
				(0, this.VSize - 3), (this.HSize - 1, this.VSize), BkColor=COLOR.BkGrey
			)
		char = ""
...
class window:
	...
	def DrawBorder(this, Color=-1):
		...
		DrawRectangle((this.MinX, this.MinY), (this.XDif, this.YDif), Color)
